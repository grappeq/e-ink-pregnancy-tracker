...
class ScreenUI:
    ...
    def coco_image(self):
        coco = Image.open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'coco-transparent.png'))
        coco_mask = Image.open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'coco-mask.png'))
        progress_bar = Image.new("L", coco.size, WHITE)

        draw = ImageDraw.Draw(progress_bar)
        draw.rectangle((0, coco.height * (1 - self.pregnancy.progress), coco.width, coco.height), fill=DARK_GRAY)
        progress_coco = Image.new("L", coco.size, WHITE)
        progress_coco.paste(progress_bar, (0, 0), coco_mask)
        progress_coco.paste(coco, (0, 0), coco)

        return progress_coco
    ...
    def _draw_carriage(self):
        carriage = Image.open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'pic/carriage.png'))
        carriage.convert("1")
    ...
    def _draw_moon(self):
        moon = Image.open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'pic/moon.png'))
        moon.convert("1")
        circle_y1 = int(self.PROGRESS_BAR_Y_CENTER - self.ICON_CIRCLE_SIZE/2)
        circle_y2 = circle_y1 + self.ICON_CIRCLE_SIZE
    ...
    def draw(self):
        ...
        # draw.text((30, 30), pregnancy.get_progress_bar(), font=font24, fill=0)
        return self._img
    ...
    def _get_progress_bar_mid_x_point(self):
        return self.ICON_X_MARGIN + self.ICON_CIRCLE_SIZE + self._get_progress_bar_length()*self.pregnancy.progress